...
// 二值信号量：防止 camera_capture() 和 camera_reinit() 并发访问
...
esp_err_t camera_init_custom(void) {
    ...
    if (s_camera_lock == NULL) {
        s_camera_lock = xSemaphoreCreateBinary();
        if (s_camera_lock == NULL) {
...
camera_fb_t *camera_capture(void) {
    ...
    if (xSemaphoreTake(s_camera_lock, 0) != pdTRUE) {
        return NULL;
    }
...
esp_err_t camera_reinit(void)
{
    ...
    xSemaphoreTake(s_camera_lock, portMAX_DELAY);
