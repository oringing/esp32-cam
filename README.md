...
- （这里是我所购买的esp32cam模块参数，购买商家如上图所示，具体参数请以自己的开发板为准）
### 🖥️ ESP32-CAM 模块核心参数
...
由于代码未优化（其实就是写的伊托答辩），生成的.bin固件大小可能超过默认分区表factory分区容量（通常为1MB），会导致如下报错：
```cpp
...
### 🎯 下一步计划
1. 实现远程截图功能，通过HTTP请求获取图片
2. 实现SD卡图片保存功能，将截图存储到TF卡
3. 完善MQTT控制功能，支持开关摄像头等指令
4. 开发内网穿透功能，实现公网访问
